...
def print_apple_cake_recipe():
    if sys.stdout.isatty():  # Only show if running interactively
        ...
        print(
            "  2. Sift the flour with baking powder into a separate bowl.",
            file=sys.stderr,
        )
        print(
            "  3. Gently fold in flour in parts. The dough should turn out like thick sour cream.",
            file=sys.stderr,
        )
        print(
            "  4. Put sliced apples into the dough. Put the dough into a mold and smooth it out.",
            file=sys.stderr,
        )
